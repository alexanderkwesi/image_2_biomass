...
-- Create index on is_active for filtering active/inactive farms
...
-- For searching farms by name
CREATE INDEX idx_farms_name ON farms(name);

-- For searching farms by location
CREATE INDEX idx_farms_location ON farms(location);

-- For compound queries (user + active status)
CREATE INDEX idx_farms_user_active ON farms(user_id, is_active);

-- For date-based queries (updated_at is frequently used in the app)
CREATE INDEX idx_farms_updated_at ON farms(updated_at DESC);

-- For combined date filtering
CREATE INDEX idx_farms_user_date ON farms(user_id, created_at DESC);



CREATE TYPE soil_type_enum AS ENUM (
    'Loam', 'Clay', 'Sandy Loam', 'Silt', 
    'Peat', 'Chalk', 'Sand', 'Unknown'
);

CREATE TYPE crop_type_enum AS ENUM (
    'Vegetables', 'Fruits', 'Grains', 'Herbs',
    'Flowers', 'Mixed Crops', 'Other'
);
